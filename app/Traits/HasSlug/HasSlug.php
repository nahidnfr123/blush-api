...
use App\Traits\HasSlug\Exceptions\InvalidOption;
use Illuminate\Database\Eloquent\Collection;
use Illuminate\Database\Eloquent\Model;
use Illuminate\Support\Str;


// A copy of the HasSlug trait from the spatie/laravel-sluggable package
trait HasSlug
{
    ...
    public function resolveRouteBinding($value, $field = null): Model|Collection|static
    {
        ...
        if ($this->slugOptions->modelBindingWithBothIdAndSlug) {
            return $this->where('id', $value)
                ->orWhere($this->slugOptions->slugField, $value)
                ->firstOrFail();
        } else if ($this->slugOptions->replaceRouteKey) {
            return $this->where($this->slugOptions->slugField, $value)
                ->firstOrFail();
        }
        if (is_numeric($value)) {
    ...
    public function getRouteKeyName(): string
    {
        ...
        return $this->slugOptions->replaceRouteKey ? $this->slugOptions->slugField : 'id';
    }
